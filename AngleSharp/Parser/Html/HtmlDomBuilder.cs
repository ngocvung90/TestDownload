            ...
            do
            {
                    ...
                    if (token.HasContent)
                    {
                        ...
                        for (int i = 0; i < tagToken.Attributes.Count; i++)
                        {
                            ...
                            if (bFoundImageLink)
                            {
                                ...
                                {
                                    listHref.Add(tagToken.Attributes[i].Value);
                                    break;
                                }
                            }
                        }
                ...
                else if (str == "img" && !isHTML)
                {
                    if (token.HasContent)
                    {
                        ...
                        for (int i = 0; i < tagToken.Attributes.Count; i++)
                        {
                            if (tagToken.Attributes[i].Value.StartsWith("/image-server") && tagToken.Attributes[i].Value.EndsWith(".png"))
                                listHref.Add(tagToken.Attributes[i].Value);
                        }
                    }
                }
                else if (str == "div" && !isHTML)
                {
                    if (token.HasContent)
                    {
                        ...
                        for (int i = 0; i < tagToken.Attributes.Count; i++)
                        {
                            if (tagToken.Attributes[i].Value == "description textMuted")
                            {
                                bFoundSubTitle = true;
                                break;
                            }

                            if (tagToken.Attributes[i].Value == "title headline secondary")
                            {
                                bFoundTitle = true;
                                break;
                            }

                            if (tagToken.Attributes[i].Value == "tags textMuted")
                            {
                                bFoundTag = true;
                                break;
                            }
